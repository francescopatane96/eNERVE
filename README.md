 ...
 ***
 ...
 2. Quality control of proteome module and generation of protein instances: proteins that do not pass the QC process are discarded (discarded_sequences.fasta);
 ...
 6. Transmembrane topology prediction module: TMHMM (Hidden Markov model). Predicts the probability of each amino acid to be 'i', 'o', or 'M'. For more information, please visit tmhmm [repository](https://github.com/dansondergaard/tmhmm.py) ;
...
```
5. create a virtual environment with the python module ```venv``` (to avoid dependencies conflicts) with: 
```
...
If you want to create a workid directory in which save outputs, please specify [-wd] (eg. -wd /workdir)
 ...
 cd eNERVE
 ```
 2. create your working directory (where you will put in fasta.file to be analyzed and where outputs will be saved);
